...
static void Turb (float *a, int iSize, DBL fTurb, DBL fScale, int iEven, int y, int x)
   {
   DBL phase, rcos, rsin, rad = 0;

   phase = 2 * M_PI * ((ran1() * 0x7FFF) / arand);
   if (x + y)
      rad = pow((DBL) (x*x + y*y), -(fTurb+1) / 2) * gauss();

   rcos = rad * cos(phase) * fScale; 
   rsin = rad * sin(phase) * fScale;

   if (iEven) // quadrants 2 and 4
      {
      Real(a, x, y)                += rcos; 
      Imag(a, x, y)                += rsin;
      Real(a, iSize-x-1,iSize-y-1) += rcos;
      Imag(a, iSize-x-1,iSize-y-1) += rsin;
      }
   else       // quadrants 1 and 3
      {
      Real(a, x, iSize-y-1)    += rcos; 
      Imag(a, x, iSize-y-1)    += rsin;
      Real(a, iSize-x-1, y)    += rcos;
      Imag(a, iSize-x-1, y)    += rsin;
      }
   }
 ...
 */
static void ApplyTurbulence(float *a, int iSize, DBL fTurb, DBL fScale)
   {
   int x, y, xcent;

   prntf("Applying Turbulence  Turb:%f Scale:%f.\n", -fTurb+3, fScale); 
   xcent = (int)(iSize / 2.0 - 0.5);  /* center dimensions of array */

   for (y = 0; y <= xcent; y++) 
      for (x=0;x<=y;x++)
         {
         Turb (a, iSize, fTurb, fScale, 1, y, x);
         Turb (a, iSize, fTurb, fScale, 1, x, y);
         Turb (a, iSize, fTurb, fScale, 0, y, x);
         Turb (a, iSize, fTurb, fScale, 0, x, y);
         }
   Imag(a, iSize/2, 0  ) = 0;
   Imag(a, 0,   iSize/2) = 0;
   Imag(a, iSize/2, iSize/2) = 0;
   }
 ...
 *       		       Peitgen & Saupe. 
 */
static float *Synthesize(UINT n)
   {
   int i,j,nsize[2];
   float *a;
   ...
   if (!(a = (float *) malloc((n+4)*(n+4)*2 * sizeof(float))))
   	Error("Cannot allocate %d x %d result array.\n", n, n);
...
static BOOL GenerateLandscape(int iSIZE)
   ...
   initgauss(uSEED);
	...
	a = Synthesize(iSIZE); // generate array via IFFT on 1/f scaled noise
	if (a == (float *) 0) 
	   return FALSE;
   ...
   if (bCRATERS) 
      {
	   ...
	   prntf("Adding %d craters of depth %f\n", num_craters, fCRATER_SCALE);

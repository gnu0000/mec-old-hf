 ...
 *
 * Fortran code by: RC Singleton, Stanford Research Institute, Sept. 1968
...
#define FFT_PI   3.14159265358979323846264338327950288
#define NFACTOR   11
...
int   fftn (int ndim, const int dims[], REAL Re[], REAL Im[],
				      int iSign, double scaling)
   {
	...
	/* tally the number of elements in the data array and determine
	 * the number of dimensions */
	for (i = 0, nTotal = 1; i < ndim; i++)
	   {
		if (dims[i] <= 0)
		   {
	...
	/* follow John Beale's example, just use the largest dimension and
	 * don't worry about excess allocation.  May be someone else will
	 * do it? */
...
int   fftradix (REAL Re[], REAL Im[],
					       ...
					       int iSign, int max_factors, int max_perm)
   {
	...
	REAL  pi2 = FFT_PI;			  /* use PI first, 2 PI later */
	...
	if (Tmp0 == NULL || Tmp1 == NULL || Tmp2 == NULL || Tmp3 == NULL || Perm == NULL)
		...
		switch (factor[ii - 1])
		   {
					...
					do
					   {
						...
						s1 *= c1;
						c1 *= c2;
						/* values of c2, c3, s2, s3 that will get used next
						 * time */
						c2 = c1 * c1 - s1 * s1;
						s2 = 2.0 * c1 * s1;
						c3 = c2 * c1 - s2 * s1;
			...
			default:
				...
				switch (k)
				   {
					case 3:			  /* transform for factor of 3
										   * (optional code) */
						do
						   {
							do
					...
					case 5:			  /* transform for factor of 5
										   * (optional code) */
						c2 = c72 * c72 - s72 * s72;
						s2 = 2.0 * c72 * s72;
						do
